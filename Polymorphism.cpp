...
#include<list>
using namespace std;
/*polymorphism is an oop concept where a method can have more than one behaviour in 2 or more classes
for example the two methods of the derived classes below print out different messages*/
...
public:
	...
	void note(){
		cout<<"This is the "<<Name<< " department"<<endl;
	}
...
class NewDepartment: public Department{	//NewDepartment is a class that inherits all of the public attributes of the Department class
public:
		NewDepartment(string name, double level): Department(name, level){}
			
	void notice(){
		cout<<"Hello FEMS"<<endl; //The methods of a derived class cannot be accessed by a base class
	}
	void note(){
		cout<<" This is the "<<Name<<" department"<<endl;
	}
...
int main ()
{
		...
		NewDepartment FEMS("faculty of economics",400);
		FEMS.note();
		FEMS.notice();
